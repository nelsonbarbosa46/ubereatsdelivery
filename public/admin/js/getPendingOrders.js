...
function getMerchantsUnchecked() {
    var url = getUrlToSubmit();
    $.ajax({
        url: url+'/api/user/getMerchantsUncheck/',
        type: 'GET',
        cache: false,
        headers: {
            "Authorization": 'Bearer ' + token
        ...
        success: function (data) {
            if (data.listUsers.length != 0) {
                ...
                for (let i = 0; i < data.listUsers.length; i++) {
                    html += '\
                    <div id="divMerchant'+data.listUsers[i].idMerchant+'" class="divDriversMerchantPending">\
                        <div class="col s2 mt-2">'+data.listUsers[i].name+'</div>\
                        <div class="col s10">\
                            <div class="col s1">\
                                <!-- Modal Trigger -->\
                                <button id="'+data.listUsers[i].idMerchant+'" class="btn mt-1 btn-small blue btnInfoMerchant"><i class="material-icons round">info</i></button>\
                            </div>\
                            <form action="#" class="checkedMerchant" id="'+data.listUsers[i].idMerchant+'"> \
                                <div class="row m-0">\
                                    <div class="col s5">\
                                        <div class="input-field col s12 m-0">\
                                    ...
                                    <div class="col s1 divButtonPending">\
                                        <button type="submit" id="'+data.listUsers[i].idMerchant+'" class="btn success waves-effect canWorkContinue"><i class="material-icons round">\
                                            check\
                                        </i></button>\
                                    </div>\
                                </div>\
                    ...
                    <div class="divClassInfoMerchant col s12" id="divInfoMerchant'+data.listUsers[i].idMerchant+'">\
                        <div class="divider"></div>\
                        <div>\
                            <h6>Informações '+data.listUsers[i].name+'</h6>\
                            <div class="row">\
                                ...
                                <div class="col s4"><p>Localidade:</p></div>\
                                <div class="col s8"><p>'+data.listUsers[i].location+'</p></div>\
                                <div class="col s4"><p>NIPC:</p></div>\
                                <div class="col s8"><p>'+data.listUsers[i].nipc+'</p></div>\
                                <div class="col s4"><p>Número de contacto:</p></div>\
                                <div class="col s8"><p>'+data.listUsers[i].contactNumber+'</p></div>\
                                <div class="col s4"><p>Descrição:</p></div>\
                                <div class="col s8"><p>'+data.listUsers[i].description+'</p></div>\
                            </div>\
                        </div>\
                    </div>\
                    <div class="col s12 divider"></div>\
                ...
                html += '</div>';
                $("#tablePendingMerchants").html(html);
            } else {
                $("#tablePendingMerchants").html("<b>Não existe empresas com pedidos pendentes</b>");
            }
        }, 
        error: function (jqXHR, textStatus, err) {
            console.log(jqXHR);
...
function submitMerchantCheck(e, id, form) {
    e.preventDefault();
   ...
   verifyFieldsFormDriverMerchantCheck(errFields, reason, reasonInput);
    ...
    if (errFields.length === 0) {
        url = getUrlToSubmit();
        $.ajax({
            url: url+'/api/user/checkMerchant/'+id,
            type: 'PUT',
            cache: false,
            headers: {
                "Authorization": 'Bearer ' + token
